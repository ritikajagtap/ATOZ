...
int main()
{
    ...
    fastio();
    vector<int> prices = {7,1, 5, 6, 8, 9};
    int minPr = prices[0];
    int mxPr = 0;
    for (int i = 0; i < prices.size(); i++)
    {
        // updating minPr when we encounter a price less than minimum price
        minPr = min(prices[i], minPr);
        // mxPr is storing the maximum difference
        mxPr = max(prices[i] - minPr, mxPr);
    }
    cout<<mxPr<<endl;
    return 0;
}